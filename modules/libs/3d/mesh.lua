...
function mesh.new(Texture,Vertices,UVs,Tris,Normals)
    ...
    function i:update(cam)
        ...
        for t=1, #tris do
            ...
            -- set or dot < 0 to make the faces correct - as it is, we drop them a little early as a stylistic thing
            if not i.culling or dot <-.08 then -- face is visible
                --get light level
                --local ldot = (-0.7)*normal[1] + (0.7)*normal[2] + (0)*normal[3]
                --ldot = ldot > -1 and ldot or -1
                --ldot = ldot < 1 and ldot or 1
                --local light = (ldot+1)/2*255
                    ...
                    --update the coordinates, and uvs of the 2d mesh point
--                    mesh:setVertex( v2id, projected3d[tris[t][v].vertexID][1], projected3d[tris[t][v].vertexID][2], uvs[tris[t][v].uvID][1], uvs[tris[t][v].uvID][2],light[tris[t][v].vertexID],light[tris[t][v].vertexID],light[tris[t][v].vertexID])
                    -- this version simulators pixed ppoint
                    local scale = 70/(70 + projected3d[tris[t][v].vertexID][3])
                    local newX = 0 + projected3d[tris[t][v].vertexID][1] * scale
                    local newY = 0 + projected3d[tris[t][v].vertexID][2] * scale
                    mesh:setVertex( v2id,
                      --maths.roundby(-projected3d[tris[t][v].vertexID][3] / projected3d[tris[t][v].vertexID][1] * FOV, 3),
                      --maths.roundby(projected3d[tris[t][v].vertexID][2], 3),
                      newX,
                      newY,
                      uvs[tris[t][v].uvID][1],
                      uvs[tris[t][v].uvID][2],
                      (light[tris[t][v].vertexID]),
                      (light[tris[t][v].vertexID]),
                      (light[tris[t][v].vertexID]))
