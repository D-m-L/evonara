...
function Map:initWorldCollision(world)
	...
	local function addObjectToWorld(objshape, vertices, name)
		local shape
    local v1, v2 = unpack(vertices)
    v1, v2 = self:convertScreenToTile(v1, v2)
    
    if name == nil then name = "block" .. v1 .. "," .. v2 end
...
--  framework.draw(self.canvas, 0, 0, 0, 1, 1)
end
...
--            framework.setColor(255, 255, 255, 255)
--            framework.setCanvas(canvas)
--            framework.draw(self.canvas, -cam.tx, -cam.ty, 0, 1, 1)
					end
				end
			end
		end
